...
description: >
  ...
  state store usage (string/binary values, key format), async tasks, or I/O Runtime integration patterns.
...
- **IO_EVENTS.md** — Load when integrating with I/O Events, Files, or external services from an action (State: see STATE_STORE.md for constraints)
- **STATE_STORE.md** — Load when using `@adobe/aio-lib-state`: key format and size limits, string/binary values only, serialize/deserialize objects
...
- State Store values are **strings or binary only**; objects need `JSON.stringify` / `JSON.parse` (see STATE_STORE.md)
